...
class XSSAttack(BaseAttack.BaseAttack):
    PORT_DESTINATION = 'port.dst'
    TARGET_HOST = 'target.host'

    template_attack_pcap_path = Util.RESOURCE_DIR + "xss.pcap"
    # HTTP port
    http_port = 80
    ...
    def __init__(self):
        """
        Creates a new instance of the XSS Attack.
        """
        # Initialize attack
        super(XSSAttack, self).__init__("XSS Attack", "Injects a XSS attack'",
                                         "Privilege elevation")
    ...
    def generate_attack_packets(self):
        ...
        for self.pkt_num, pkt in enumerate(exploit_raw_packets):
            ...
            if ip_pkt.getfieldval("dst") == orig_ip_dst:  # victim IP
                ...
                str_tcp_seg = self.modify_http_header(str_tcp_seg, '/', target_uri, orig_ip_dst, target_host)
